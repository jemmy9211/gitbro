...
@click.group()
@click.version_option(version="2.0.0", prog_name="gitbro")
@click.pass_context
def cli(ctx):
...
def commit(temperature, auto, conventional):
    ...
    try:
        ...
        if not provider:
            click.echo("No AI provider configured. Run 'gitbro setup' first.", err=True)
            sys.exit(1)
...
echo "🤖 Running gitbro validation..."
...
from cli import cli
sys.argv = ['gitbro', 'validate', '--range', 'HEAD~1..HEAD', '--conventional']
cli()
"
...
def setup(provider):
    ...
    if provider:
        sys.argv = ['gitbro', '--setup', provider]
    else:
        sys.argv = ['gitbro', '--setup']
...
def _generate_alias():
    ...
    # Basic alias
    basic_alias = "alias git-clean-branches='gitbro clean-branches'"
    click.echo(f"# Basic cleanup alias")
    click.echo(basic_alias)
    click.echo("")
    
    # Advanced alias with options
    advanced_alias = "alias git-clean-all='gitbro clean-branches --remote'"
    click.echo(f"# Advanced cleanup (includes remote branches)")
    click.echo(advanced_alias)
    click.echo("")
    
    # Dry run alias
    dry_run_alias = "alias git-clean-preview='gitbro clean-branches --dry-run --remote'"
